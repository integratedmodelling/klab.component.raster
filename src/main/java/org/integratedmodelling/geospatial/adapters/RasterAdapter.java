...
@ResourceAdapter(
    ...
    universal = true,
    parameters = {
      @Parameter(name = "noData", type = Artifact.Type.NUMBER, optional = true)
    })
public class RasterAdapter {
  ...
  @Importer(
      ...
      fileExtensions = {"tiff"})
  public static Resource importGeotiff(String urn, File file, Parameters<String> userParameters) throws IOException {
    ResourceBuilderImpl builder = validateLocalImport(urn, file, Parameters.create());
  ...
  public static ResourceBuilderImpl validateLocalImport(String urn, File file, Parameters<String> userData) throws IOException {
    ResourceBuilderImpl ret = new ResourceBuilderImpl(urn);
    ret.withParameters(userData);
    ret.withResourceVersion(Version.CURRENT_VERSION); // Or start with a default value
    ...
    // TODO set geometry
    Geometry geometry = Geometry.builder().space()
            .boundingBox(envelope.getMinimum(0), envelope.getMaximum(0), envelope.getMinimum(1), envelope.getMaximum(1))
            .build().build();
  ...
  public static void main(String[] args) throws Exception {
    ...
    File file = new File("./src/test/java/resources/multibandTest.tiff");
    if (!file.exists()) {
      throw new Exception();
    }
    var result = RasterAdapter.importGeotiff("aaa", file, Parameters.create());
    System.out.println(result.getAdapterType());
